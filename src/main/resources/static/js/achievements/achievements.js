...
// 未登录时点击登录按钮
function loginS() {
    window.location.href = "http://localhost:8080/Login?flag=achievements";
}
// 删除浏览器中的cookie（根据名字删除）
...
// 显示清晰图片的模态窗口函数
function showImgInModal(img, modalId) {
    // 获取模态窗口
    let modal = $('#' + modalId);
    // 图片模态窗
    let modalImg = $('#' + modalId).children('.modal-content'); // $('#' + modalId + ' img')
    // 获取模态窗口中显示文字注解的div
    let captionText = $('#' + modalId).children('#caption');
    // 图片点击后显示模态框
    modal.css('display', 'block');
    // 显示图片
    modalImg.attr('src', img.src);
    // 给模态框中的图片加上alt
    modalImg.attr('alt', img.alt);
    // 显示文字注解
    captionText.text(img.alt);
    // 获取 <span> 元素并设置关闭模态框按钮（lambda表达式写法）
    modal.children('.close').click(() => {
                                    ...
                                    setTimeout(function () {
                                        swal({
                                            ...
                                            function(isConfirm){
                                                if (isConfirm) {
                                                    // 刷新整个页面
                                                    window.location.href = "http://localhost:8080/getpage?value=achievements";
                                                    // 局部刷新
